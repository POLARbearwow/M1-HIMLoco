...
DEFAULT_CONFIG = MUJOCO_DIR / "configs" / "m1.yaml"
...
class DeployM1:
    ...
    def _print_startup_summary(self):
        print(f"[M1 Deploy] Config: {self.cfg['config_path']}")
        print(f"[M1 Deploy] Scene : {self.xml_path}")
        print(f"[M1 Deploy] ONNX  : {self.policy_path}")
        print(f"[M1 Deploy] ONNX input dim : {self.input_dim}")
        print(f"[M1 Deploy] ONNX output dim: {self.output_dim}")
        print(f"[M1 Deploy] Train DOF order : {self.train_dof_names}")
        print(f"[M1 Deploy] MuJoCo qpos order: {self.mujoco_joint_names}")
        print(f"[M1 Deploy] MuJoCo actuator order: {self.actuator_joint_names}")
        print(f"[M1 Deploy] policy_to_qpos    : {self.policy_to_qpos}")
        print(f"[M1 Deploy] policy_to_actuator: {self.policy_to_actuator}")
        print("[M1 Deploy] Actor obs layout: ang_vel(3) + gravity(3) + cmd(3) + dof_err(16) + dof_vel(16) + last_action(16) = 57")
        print(f"[M1 Deploy] Obs history dim: {self.expected_obs_history_dim}")
        print("[M1 Deploy] Keyboard fallback: W/S vx, Q/E vy, A/D yaw, Space zero cmd, R reset")
    ...
    def _sync_command(self):
        if self.keyboard_state.clear_command:
            ...
            print("[M1 Deploy] Cleared command.")
            return
    ...
    def reset(self, full_reset=False):
        ...
        if full_reset:
            self.control_step = 0
        print("[M1 Deploy] Reset to default_pos keyframe.")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Deploy HIMLoco M1 policy in MuJoCo.")
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    ...
    deploy = DeployM1(cfg, onnx_override=args.onnx)
    try:
        deploy.run()
    finally:
        deploy.shutdown()
